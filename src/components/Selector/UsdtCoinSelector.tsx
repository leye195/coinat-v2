...
interface UsdtCoinSelectorProps {
  code?: string;
}

const UsdtCoinSelector = ({ code }: UsdtCoinSelectorProps) => {
  const { usdtCoinData } = useCoinList();
  ...
  useLayoutEffect(() => {
    if (code && usdtCoinData) {
      const info = usdtCoinData.find(({ name }) => name === code);
...
export default UsdtCoinSelector;
