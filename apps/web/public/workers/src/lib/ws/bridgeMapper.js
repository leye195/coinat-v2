export const bookKey = (t) => `${t.exchange}:${t.symbol}:${t.quote}`;
export function createBook() {
    return new Map();
}
export function applyTicker(book, t) {
...
export function buildPayload(book) {
    const payload = {
    ...
    for (const t of book.values()) {
        ...
        if (t.exchange === 'upbit') {
            if (t.quote === 'KRW') {
                ...
                if (sym === 'BTC')
                    payload.upbit.btcKrw = t.tradePrice;
            }
            else if (t.quote === 'BTC') {
