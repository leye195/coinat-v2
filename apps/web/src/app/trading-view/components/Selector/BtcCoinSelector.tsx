...
interface BtcCoinSelectorProps {
  code?: string;
}

const BtcCoinSelector = ({ code }: BtcCoinSelectorProps) => {
  const { btcCoinData } = useCoinList();
  ...
  useLayoutEffect(() => {
    if (code && btcCoinData) {
      const info = btcCoinData.find(({ name }) => name === code);
...
export default BtcCoinSelector;
